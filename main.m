...
for im_num = 1:15
    ...
    [DisparityMap{1}, DisparityMap{2}] = Pakal(I{1}, I{2}, DisparityRange(1,2));
    [DisparityMap_sparse{1}, DisparityMap_sparse{2}] = stereoConsistencyCheck(DisparityMap{1}, DisparityMap{2},1);
       
    %---------------------------------------------------------------------%
    time_taken = toc;
    figure, imshow([DisparityMap{1}/DisparityRange(2),DisparityMap_sparse{1}/DisparityRange(2)]);
    drawnow;
